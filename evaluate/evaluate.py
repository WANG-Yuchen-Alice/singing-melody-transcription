    ...
    if time_shift >= 0.0:
        ...
        for i in range(len(answer_pred)):
            if (answer_pred[i] is not None and float(answer_pred[i][1]) - float(answer_pred[i][0]) > 0
                ...
                est_intervals.append(
                    [answer_pred[i][0]+time_shift, answer_pred[i][1]+time_shift])
        ...
        for i in range(len(answer_true)):
            if (answer_true[i] is not None and float(answer_true[i][1]) - float(answer_true[i][0]) > 0
                ...
                ref_intervals.append(
                    [answer_true[i][0]-time_shift, answer_true[i][1]-time_shift])
                ref_pitches.append(answer_true[i][2])
...
def eval_one_data(answer_true, answer_pred, onset_tolerance=0.05, shifting=0, gt_pitch_shift=0):
    ...
    est_pitches = np.array([float(est_pitches[i])
    ...
    raw_data = transcription.evaluate(ref_intervals, ref_pitches, est_intervals,
                                      est_pitches, onset_tolerance=onset_tolerance, pitch_tolerance=50)
...
def eval_all(answer_true, answer_pred, onset_tolerance=0.05, shifting=0, print_result=True, id_list=None):
    ...
    for i in range(len(answer_true)):
        ret = eval_one_data(
            answer_true[i], answer_pred[i], onset_tolerance=onset_tolerance, shifting=shifting, gt_pitch_shift=0.0)
