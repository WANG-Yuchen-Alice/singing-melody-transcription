...
class EffNetPredictor:
    def __init__(self, device="cuda:0", model_path=None):
        ...
        if model_path is not None:
            ...
            self.model.load_state_dict(torch.load(
                model_path, map_location=self.device), strict=False)
    ...
    def fit(self, train_dataset_path, valid_dataset_path, model_dir, **training_args):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                onset_logits, offset_logits, pitch_octave_logits, pitch_class_logits = self.model(
                    input_tensor)

                split_train_loss0 = self.onset_criterion(
                    onset_logits, onset_prob)
                split_train_loss1 = self.offset_criterion(
                    offset_logits, offset_prob)
                split_train_loss2 = self.octave_criterion(
                    pitch_octave_logits, pitch_octave)
                split_train_loss3 = self.pitch_criterion(
                    pitch_class_logits, pitch_class)

                total_split_loss[0] = total_split_loss[0] + \
                    split_train_loss0.item()
                total_split_loss[1] = total_split_loss[1] + \
                    split_train_loss1.item()
                total_split_loss[2] = total_split_loss[2] + \
                    split_train_loss2.item()
                total_split_loss[3] = total_split_loss[3] + \
                    split_train_loss3.item()

                loss = split_train_loss0 + split_train_loss1 + \
                    split_train_loss2 + split_train_loss3
                loss.backward()
                self.optimizer.step()
                total_training_loss += loss.item()

                if batch_idx % 5000 == 0 and batch_idx != 0:
                    print(epoch, batch_idx, "time:", time.time()-start_time,
                          "loss:", total_training_loss / (batch_idx+1))
            ...
            if epoch % self.save_every_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    for batch_idx, batch in enumerate(self.valid_loader):
                        ...
                        split_val_loss0 = self.onset_criterion(
                            onset_logits, onset_prob)
                        split_val_loss1 = self.offset_criterion(
                            offset_logits, offset_prob)

                        split_val_loss2 = self.octave_criterion(
                            pitch_octave_logits, pitch_octave)
                        split_val_loss3 = self.pitch_criterion(
                            pitch_class_logits, pitch_class)

                        split_val_loss[0] = split_val_loss[0] + \
                            split_val_loss0.item()
                        split_val_loss[1] = split_val_loss[1] + \
                            split_val_loss1.item()
                        split_val_loss[2] = split_val_loss[2] + \
                            split_val_loss2.item()
                        split_val_loss[3] = split_val_loss[3] + \
                            split_val_loss3.item()
                ...
                target_model_path = Path(
                    self.model_dir) / (training_args['save_prefix']+'_{}'.format(epoch))
        ...
        print('Training done in {:.1f} minutes.'.format(
    ...
    def _parse_frame_info(self, frame_info, onset_thres, offset_thres):
        ...
        last_onset = 0.0
        onset_seq = np.array([frame_info[i][0]
            ...
            if info[0] >= onset_thres and onset_seq[i] == np.amax(onset_seq[backward_frames: forward_frames]):
                ...
                if current_onset is None:
                    ...
                    if len(pitch_counter) > 0:
                        result.append([current_onset, current_time, max(
                            set(pitch_counter), key=pitch_counter.count) + 36])
    ...
    def predict(self, test_dataset, results={}, onset_thres=0.1, offset_thres=0.5):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(test_loader)):
                ...
                onset_probs, offset_probs = torch.sigmoid(
                    onset_logits).cpu(), torch.sigmoid(offset_logits).cpu()
                ...
                for bid, song_id in enumerate(song_ids):
                    frame_info = (onset_probs[bid], offset_probs[bid], torch.argmax(
                        pitch_octave_logits[bid]).item(), torch.argmax(pitch_class_logits[bid]).item())
            ...
            for song_id, frame_info in song_frames_table.items():
                results[song_id] = self._parse_frame_info(
                    frame_info, onset_thres=onset_thres, offset_thres=offset_thres)
        return results
